...
public class CreditScoreDiscountRule implements RuleEvaluator {
    ...
    public boolean matches(PriceRequest priceRequest, String action, double rateDiscount, double creditScoreRule, String creditScoreMathSymbolRule, String personStateRule, String productNameRule) {
        ...
        return ((priceRequest.getCredit_score() >= RulesConstant.CURRENT_CREDIT_SCORE_BAR)
                || (creditScoreRule != 0.0 && creditScoreMathSymbolRule != null
                && creditScoreMathSymbolRule.equalsIgnoreCase(RulesConstant.GREATER_THAN_OR_EQUAL_TO)));
    }
    ...
    public Result evaluate(PriceRequest priceRequest, String action, double rateDiscount, double creditScoreRule, String creditScoreMathSymbolRule, String personStateRule, String productNameRule) {
        ...
        if (priceRequest.getCredit_score() >= RulesConstant.CURRENT_CREDIT_SCORE_BAR) {
            result.setInterest_rate(RulesConstant.START_INTEREST_RATE - RulesConstant.POINT_THREE_PERCENT_INCREASED_RATE);
        ...
        if (creditScoreRule != 0.0 && creditScoreMathSymbolRule != null && creditScoreMathSymbolRule.equalsIgnoreCase(RulesConstant.GREATER_THAN_OR_EQUAL_TO)) {

            if (priceRequest.getCredit_score() >= creditScoreRule && action.equalsIgnoreCase(RulesConstant.DECREASE_RATE)) {
